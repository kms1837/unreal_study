...
public:
	...
	UParticleSystem* MarkBurnParticle;
	...
	FText GetAmmunitionInfoText();

	UFUNCTION(BlueprintPure)
	FText GetShotModeText();
...
protected:
	...
	const float MIN_BOUND_RANGE = 0.1f;
	const float MAX_BOUND_RANGE = 2.f;
	float bundRange = 0;

	int score = 0;

	enum { HAND_GUN=0, MACHINE_GUN=1, SHOT_GUN=2};
	enum { SEMI_AUTO=0, THREE_ROUND_BURST=1, FULL_AUTO=2 };
